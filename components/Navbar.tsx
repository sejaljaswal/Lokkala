            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-16 items-center">
                    ...
                    <div className="flex-shrink-0 flex items-center">
                        <Link href="/" className="text-2xl font-bold bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent transition-transform hover:scale-105">
                            Lokkala
                        </Link>
                    </div>
                        ...
                        {navLinks.map((link) => (
                            link.name === "Login" ? (
                                <Link
                                    key={link.name}
                                    href={link.href}
                                    className="bg-indigo-600 text-white px-5 py-2 rounded-full font-medium hover:bg-indigo-700 transition-all duration-300 shadow-md hover:shadow-indigo-200"
                                >
                                    {link.name}
                                </Link>
                            ) : (
                    ...
                    {navLinks.map((link) => (
                        <Link
                            ...
                            className={`block px-3 py-2 rounded-md text-base font-medium transition-all ${link.name === "Login"
                                    ? "bg-indigo-600 text-white hover:bg-indigo-700 mt-4 mx-3 text-center"
